let helper = function (grid, x, y) {
    ...
    if (x === xlen) {
        return grid[y][x] + helper(grid, x, y + 1)
    }

    if (y === ylen) {
        return grid[y][x] + helper(grid, x + 1, y)
    }

    return grid[y][x] + Math.min(helper(grid, x + 1, y), helper(grid, x, y + 1))
}
